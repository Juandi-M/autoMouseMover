...
def setup_navigation_frame(app):
    ...
    try:
        us_img = customtkinter.CTkImage(
            light_image=Image.open(us_flag_path),
            dark_image=Image.open(us_flag_path),
            size=(32, 32)  # Slightly larger flags since no text
        )
        es_img = customtkinter.CTkImage(
            light_image=Image.open(es_flag_path),
            dark_image=Image.open(es_flag_path),
            size=(32, 32)
        )
...
# Flag buttons with updated language switching
