    ...
    def add_thread(self, name: str, target, args=()) -> threading.Event:
        ...
        with self.lock:
            ...
            thread = threading.Thread(
                target=target, args=args + (stop_flag,), name=name, daemon=True
            )
...
class MouseMoverThreadManager(BaseThreadManager):
    ...
    def start_threads(self):
        ...
        # Add timer thread
        self.add_thread("timer", self._update_time, (start_perf_counter,))

        # Add mouse mover thread
        self.add_thread("mouse_mover", self._move_mouse, (start_perf_counter,))
